...
6. **技术特性**
   ...
   - 现代化UI界面
   - 单二进制文件打包(embed静态资源)
   - PBKDF2密钥派生配合真正随机盐值生成，增强安全性
   - 解密时只使用映射表中记录的盐值，确保解密一致性
   - 加解密操作无需二次确认，提升操作效率
...
- 映射表实时更新显示，无需手动刷新
- 映射表显示完整信息(MD5校验、目标目录、盐值)
- 优化的映射表布局，无横向滚动条，信息均匀分布
- 加解密操作无需二次确认，提升操作效率
...
- 加密时自动生成随机盐值（16字节）并保存至映射表，解密时从映射表中读取盐值确保一致性，同时生成随机初始化向量(IV)（16字节）
- 使用PKCS#7填充方案
- 支持流式处理，适合大文件加密
